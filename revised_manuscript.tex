...
\begin{itemize}
    ...
    \item MATSim validation is framed as a car-only road-assignment and count-matching test, not a full mode-choice model.
\end{itemize}
...
The purpose of the methodology is not to infer the total population count. In practice, population and trip totals should come from external controls such as survey expansion weights, census products, traffic counts, or planning forecasts. Our objective is narrower and more practical: generate a population-scale set of plausible trip records whose distributions and spatial-temporal patterns can be used as road-assignment demand in an agent-based simulator. The downstream MATSim exercise is correspondingly framed as a Stage 1 car-only network-loading validation against observed traffic counts. It evaluates route loading and spatial count patterns, not multimodal choice or long-run MATSim replanning equilibrium.
...
\node (counts) [below=of matsim] {MDOT SHA AADT count comparison};
\draw[->] (survey) -- (prep);
\draw[->] (prep) -- (vae);
\draw[->] (vae) -- (sample);
\draw[->] (sample) -- (valid);
...
\]
The numeric vector and all categorical embeddings are concatenated and passed through fully connected encoder layers of sizes 2048 and 1920 with layer normalization, ReLU activations, and dropout of 0.05. The encoder outputs a latent mean vector $\mu$ and log-variance vector $\log\sigma^2$ in an 1800-dimensional latent space. The decoder mirrors the hidden layers and produces one numeric head plus one categorical softmax head per categorical column.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    node distance=.35cm,
    every node/.style={draw, rectangle, font=\small, text width=4.2cm, align=center, inner sep=4pt},
    >=Stealth
]
...
\node (enc1) [below=of embed] {Encoder: Linear(2048) + LayerNorm + ReLU + Dropout};
\node (enc2) [below=of enc1] {Encoder: Linear(1920) + LayerNorm + ReLU + Dropout};
\node (latent) [below=of enc2] {Latent $\mu$ (1800), $\log\sigma^2$ (1800)};
\node (sample) [below=of latent] {$z=\mu+\sigma\epsilon$, $\epsilon\sim\mathcal{N}(0,I)$};
\node (dec1) [below=of sample] {Decoder: Linear(1920) + LayerNorm + ReLU + Dropout};
\node (dec2) [below=of dec1] {Decoder: Linear(2048) + LayerNorm + ReLU + Dropout};
\node (out) [below=of dec2] {Numeric reconstruction + categorical softmax heads};
\draw[->] (input) -- (embed);
\draw[->] (embed) -- (enc1);
\draw[->] (enc1) -- (enc2);
\draw[->] (enc2) -- (latent);
...
The reported training run uses 5040 epochs, batch size 1024, learning rate $10^{-3}$, AdamW optimization, and random seed 2026. Figure~\ref{fig:training-loss} shows that the total loss, numeric loss, categorical loss, and KL term stabilize over training.
...
Each MATSim agent in this Stage 1 setup represents one vehicle trip: an origin activity at the origin TAZ centroid, a car leg, and a destination activity at the destination TAZ centroid. The network is built from OpenStreetMap Maryland road data \cite{openstreetmapOpenStreetMap}. Observed traffic counts are obtained from Maryland State Highway Administration annual average daily traffic (AADT) count points \cite{MDOTSHAA72:online}. Count points are matched to nearest network links within \SI{100}{m}; the statewide OSM build matched \num{8762} of \num{8773} count points.

Two levels of validation are reported. First, a Maryland-internal comparison scales weighted real and synthetic road demand to the same reduced vehicle-trip total, \num{218004.4}, allowing a direct comparison of assigned volumes. Second, county-level population-scale runs compare weighted real survey demand and VAE synthetic demand separately for Maryland counties with sufficient observed counts and runnable demand. These two scenarios are never mixed: the ``real'' scenario uses weighted survey trips, while the ``synthetic'' scenario uses VAE-generated trips.

Model performance is evaluated with distributional overlap, total variation distance, mean absolute error (MAE), root mean squared error (RMSE), mean absolute percentage error (MAPE), Pearson correlation, GEH statistics, and spatial pattern metrics. Distributional overlap is defined as
\[
O(P,Q)=\sum_b \min(P_b,Q_b)=1-\mathrm{TVD}(P,Q),
\]
where $P_b$ and $Q_b$ are real and synthetic proportions in bin or category $b$. GEH is computed as
\[
\mathrm{GEH}=\sqrt{\frac{2(M-C)^2}{M+C}},
\]
where $M$ is modeled volume and $C$ is observed count volume.
...
\subsection{Maryland-Internal MATSim Stage 1 Validation}
The Maryland-internal road-assignment comparison uses the same total scaled vehicle demand for the weighted survey scenario and the synthetic scenario: \num{218004.4} vehicle trips. The input demand distributions are reasonably close. Overlap is 0.966 for travel mode, 0.931 for destination activity, 0.904 for origin activity, 0.902 for departure hour, 0.852 for activity pairs, 0.744 for origin TAZ, and 0.733 for destination TAZ.

\begin{figure}[H]
\centering
\includegraphics[width=.88\linewidth]{fig-hourly-vehicle-demand.png}
\caption{\textbf{Weighted real and synthetic vehicle trips by departure hour for the Maryland-internal Stage 1 MATSim validation.}}
\label{fig:hourly-demand}
\end{figure}

Figure~\ref{fig:hourly-demand} shows that the synthetic demand captures the daily rise and evening decline but smooths the morning peak and shifts some demand later in the day. This smoothing is typical of generative models trained to reproduce aggregate distributions while sampling from a continuous latent space.

The count validation uses \num{8745} matched AADT count links. Table~\ref{tab:stage1-counts} reports the main metrics. The synthetic scenario has higher Pearson correlation with observed counts than the weighted real scenario, but the weighted real scenario has lower MAE, RMSE, MAPE, and a slightly higher share of links with GEH below 5. Both scenarios underload total observed AADT because this Stage 1 setup uses a reduced Maryland-internal car-demand subset rather than a fully calibrated regional model.

\begin{table}[H]
\centering
\caption{Maryland-internal MATSim count validation against scaled observed AADT counts.}
\label{tab:stage1-counts}
\footnotesize
\begin{tabular}{lrrrrr}
\toprule
\textbf{Scenario} & \textbf{MAE} & \textbf{RMSE} & \textbf{MAPE} & \textbf{Correlation} & \textbf{GEH $<5$ share} \\
\midrule
Weighted survey & 528.35 & 1209.94 & 0.968 & 0.461 & 0.079 \\
VAE synthetic & 556.89 & 1326.64 & 1.224 & 0.507 & 0.071 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=.70\linewidth]{fig-assigned-vs-observed.png}
\caption{\textbf{Assigned and observed count volumes in the Maryland-internal Stage 1 comparison.}}
\label{fig:assigned-observed}
\end{figure}

\subsection{County-Level Count and Spatial-Pattern Tests}
The county-level validation compares weighted real survey demand and population-scale VAE synthetic demand against observed AADT counts in completed county runs. The current completed set includes Anne Arundel, Calvert, Carroll, Charles, Frederick, Howard, Montgomery, Prince George's, and St. Mary's counties. These are not held-out training counties; rather, they provide an external traffic-count validation of the generated demand.

Figure~\ref{fig:county-winners} summarizes which input source performs better by county and metric. Weighted survey inputs outperform synthetic inputs on absolute volume-error metrics in nearly all county comparisons: real wins MAE in 9 of 9 counties, RMSE in 8 of 9, MAPE in 9 of 9, GEH$<5$ share in 8 of 9, and total-volume error in 9 of 9. This is expected because direct weighted survey expansion remains closer to observed aggregate scale where the survey sample is available.

\begin{figure}[H]
\centering
\includegraphics[width=.92\linewidth]{fig-county-metric-winners.png}
\caption{\textbf{County-level metric winners for weighted survey and VAE synthetic demand.}}
\label{fig:county-winners}
\end{figure}

The synthetic demand is more competitive on spatial pattern metrics. It wins Pearson correlation in 6 of 9 counties and log-Pearson correlation in all 9 completed counties. Paired spatial-pattern tests also show synthetic wins for Spearman rank correlation in 7 of 9 counties, top-count-link capture in 7 counties with 1 tie, and top-count-link Jaccard similarity in 7 counties with 1 tie. Figure~\ref{fig:spearman-diff} illustrates the Spearman differences by county.

\begin{figure}[H]
\centering
\includegraphics[width=.86\linewidth]{fig-spearman-diff-by-county.png}
\caption{\textbf{Synthetic-minus-real Spearman differences for county spatial count patterns. Positive values favor the VAE synthetic demand.}}
\label{fig:spearman-diff}
\end{figure}

The pooled county scatter in Figure~\ref{fig:pooled-county-scatter} reinforces the same conclusion. Neither input source is yet calibrated to absolute observed AADT magnitudes, but the synthetic input often preserves relative spatial structure as well as, or better than, direct survey expansion. This is a useful property for enriching sparse microdata, but it is not sufficient for final planning forecasts without external volume controls.

\begin{figure}[H]
\centering
\includegraphics[width=.70\linewidth]{fig-pooled-county-assigned-vs-observed.png}
\caption{\textbf{Pooled county modeled-versus-observed count volumes across completed county validations.}}
\label{fig:pooled-county-scatter}
\end{figure}

\subsection{Scope and Limitations}
The revised methodology directly addresses several weaknesses of the earlier implementation. First, nominal variables are no longer treated as ordered integers under a global mean squared error loss. Second, population-scale trip counts are fixed by survey weights, avoiding the earlier issue of synthetic population totals drifting from known controls. Third, the MATSim experiment is explicitly scoped as a car-only road-assignment validation, rather than a full multimodal activity-based simulation.

Important limitations remain. The VAE is trained on the same survey geography used in validation, so the present county experiments do not constitute a strict spatial hold-out transferability test. A stronger future design should train on selected high-quality counties and evaluate on withheld counties with sufficient independent count data. The current MATSim agents are single vehicle-trip agents located at TAZ centroids, not full-day household activity plans with within-zone location sampling. Mode choice is synthesized as an attribute but not modeled endogenously in MATSim. Finally, absolute count performance remains weak without demand calibration, external origin--destination controls, and a complete regional multimodal network.
...
The population-scale synthetic dataset matches the weighted survey trip total by construction and reproduces many major univariate distributions, including travel mode, vehicle occupancy, activity purpose, and reported travel time. Spatial TAZ fields and sparse cross-marginals remain harder, which is expected in high-cardinality origin--destination synthesis. In MATSim road-assignment validation, synthetic demand improves several spatial-pattern metrics and correlations with observed count volumes, while weighted survey inputs still dominate absolute volume-error metrics.

The results support a cautious interpretation: mixed-type VAEs are promising for enriching trip microdata and preserving relative spatial patterns, but they should be combined with known population controls, count calibration, and multimodal choice modeling before use in policy-sensitive forecasting. Future work should add explicit hold-out transferability tests, condition generation on external demographic and OD controls, sample coordinates within TAZ polygons, reconstruct full-day person plans, and include transit and mode-choice validation.
