...
Input: synthesized-household-trips.csv
Outputs:
- synthetic_trips.csv
...
def trip_count_from_person(row: Dict[str, str], pfx: str, trip_cols: Sequence[str], od_cols: Sequence[str]) -> int:
    lengths: List[int] = []

    for c in trip_cols:
        vals = parse_tuple_like(row.get(pfx + c, ""))
        if vals:
...
def count_households(input_csv: Path) -> int:
    n = 0
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Untuple synthesized-household-trips.csv")
    ap.add_argument("--input", type=Path, default=Path("synthesized-household-trips.csv"))
    ap.add_argument("--output", type=Path, default=Path("synthetic_trips.csv"))
    ...
    total_households = count_households(args.input)
    ...
    with args.input.open("r", newline="", encoding="utf-8-sig") as f:
        ...
        # Keep only columns that exist in input.
        ...
        with args.output.open("w", newline="", encoding="utf-8") as fo, args.sample_output.open(
            "w", newline="", encoding="utf-8"
        ) as fs:
            writer_all = csv.DictWriter(fo, fieldnames=out_cols)
            writer_sample = csv.DictWriter(fs, fieldnames=out_cols)
            ...
            for hh_row in tqdm(
                ...
                household_values = {
                    "area_type": hh_row.get("area_type", "").strip(),
                    "food_delivered": hh_row.get("food_delivered", "").strip(),
                    "hh_income_detailed": hh_row.get("hh_income_detailed", "").strip(),
                    "hhsize": hh_row.get("hhsize", "").strip(),
                    "home_bmc_taz": hh_row.get("home_bmc_taz", "").strip(),
                    "home_ownership": hh_row.get("home_ownership", "").strip(),
                    "home_state_puma": hh_row.get("home_state_puma", "").strip(),
                    "home_tpb_taz": hh_row.get("home_tpb_taz", "").strip(),
                    "home_type": hh_row.get("home_type", "").strip(),
                    "numbicycle": hh_row.get("numbicycles", "").strip(),
                    "numvehicle": hh_row.get("numvehicles", "").strip(),
                    "numworkers": hh_row.get("numworkers", "").strip(),
                    "package_delivered": hh_row.get("package_delivered", "").strip(),
                    "service_delivered": hh_row.get("service_delivered", "").strip(),
                    "tdate_string": hh_row.get("tdate_string", "").strip(),
                    "wthhfin": hh_row.get("wthhfin", "").strip(),
                    "home_fips": hh_row.get("home_fips", "").strip(),
                }
                ...
                for p in p_slots:
                    ...
                    # Skip empty person slots
                    if not any((hh_row.get(pfx + c, "").strip() for c in PERSON_IN_BASE_COLS + trip_chain_cols + od_cols)):
                        continue
                    ...
                    new_person_id += 1
                    person_values = {
                        "age": hh_row.get(pfx + "age", "").strip(),
                        "disability": hh_row.get(pfx + "disability", "").strip(),
                        "employment_status": hh_row.get(pfx + "employment_status", "").strip(),
                        "gender": hh_row.get(pfx + "gender", "").strip(),
                        "jobs_count": hh_row.get(pfx + "job_count", "").strip(),
                        "license": hh_row.get(pfx + "license", "").strip(),
                        "person_tripcount": hh_row.get(pfx + "person_tripcount", "").strip(),
                        "race_ethnicity_hispanic": hh_row.get(pfx + "race_ethnicity_hispanic", "").strip(),
                        "race_ethnicity": hh_row.get(pfx + "race_ethnicity", "").strip(),
                        "smartphone": hh_row.get(pfx + "smartphone", "").strip(),
                        "student_status": hh_row.get(pfx + "student_status", "").strip(),
                        "td_shop_time": hh_row.get(pfx + "td_shoptime", "").strip(),
                        "td_telecommute_time": hh_row.get(pfx + "td_telecommute_time", "").strip(),
                        "volunteer_status": hh_row.get(pfx + "volunteer_status", "").strip(),
                        "walk_bike_loop_trips": hh_row.get(pfx + "walk_bike_loop_trips", "").strip(),
                        "work_bmc_taz": hh_row.get(pfx + "work_bmc_taz", "").strip(),
                        "work_fips": hh_row.get(pfx + "work_fips", "").strip(),
                        "work_tpb_taz": hh_row.get(pfx + "work_tpb_taz", "").strip(),
                        "school_bmc_taz": hh_row.get(pfx + "school_bmc_taz", "").strip(),
                        "school_fips": hh_row.get(pfx + "school_fips", "").strip(),
                        "school_mode": hh_row.get(pfx + "school_mode", "").strip(),
                        "school_tpb_taz": hh_row.get(pfx + "school_tpb_taz", "").strip(),
                        "school_type": hh_row.get(pfx + "school_type", "").strip(),
                    }
                    for j1 in j1_cols:
                        person_values[j1] = hh_row.get(pfx + j1, "").strip()
                    ...
                    for tc in trip_chain_cols:
                        seq_cache[tc] = parse_tuple_like(hh_row.get(pfx + tc, ""))
                    for od in od_cols:
                        seq_cache[od] = parse_tuple_like(hh_row.get(pfx + od, ""))
                    ...
                    if trip_count > 0:
                        ...
                        for k in range(1, trip_count):
                            delta_k = (
                                parse_minutes_value(dep_delta_seq[k])
                                if k < len(dep_delta_seq)
                                else None
                            )
                    ...
                    for t in range(trip_count):
                        ...
                        for tc in trip_chain_cols:
                            if tc == "vehicle":
                                vehicle_item = (
                                    seq_cache[tc][t] if t < len(seq_cache[tc]) else ""
                                )
                        ...
                        writer_all.writerow(out)
                        if write_sample:
                            writer_sample.writerow(out)
